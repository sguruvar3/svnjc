...
/**
 * This class intends to simulate "svn Cleanup" command using SVNKit Java
 * Library
 * 
...
public class Cleanup {
	...
	/**
	 * simulates the "svn Cleanup" command
	 * 
	 * @param prop
	 */
	public static void executeCleanupProcess(Properties prop, Client client,
			File[] wcDir) {
		// setup repo factory
		...
		for (File file : wcDir) {
			if (!file.exists()) {
				showError("the destination directory '"
						+ file.getAbsolutePath() + "' not exists!", null);
			}

			try {
				SVNWCClient svnWcClient = svnClientManager.getWCClient();
				svnWcClient.setIgnoreExternals(true);
				// signature : void doCleanup(File path, boolean
				// deleteWCProperties)
				svnWcClient.doCleanup(file, true);
			} catch (SVNException svne) {
				showError(
						"error while Cleaning up local changes inside working copy ",
						svne);
			}
		}
	}
	...
	public static void main(String args[]) {
		if (args.length < 1) {
			...
			logger.debug("Usage: java Cleanup <filename(s)>");
			System.exit(256);
		} else {
			try {
				...
				executeCleanupProcess(prop, client, filenames);
